...
var filePath = path.join(__dirname, 'bundleSize.json');
function fileSizeInKilobytes(path) {
    return __awaiter(this, void 0, void 0, function () {
        var stats;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4, stat(path)];
                case 1:
                    stats = _a.sent();
...
function getPreviousBundleSize() {
    ...
    if (fs.existsSync(filePath)) {
        var sizeFile = fs.readFileSync(filePath);
...
function reportBundleSize(previousBundleSize, currentBundleSize) {
    ...
    if (typeof currentBundleSize === 'number') {
        var currentBundleSizeText = cyan("".concat(Math.round(currentBundleSize), " kB"));
        console.log("\nBundle size: ".concat(currentBundleSizeText));
    }
    if (typeof difference === 'number') {
        var percentageText = difference > 0 ? red("".concat(difference, "%")) : green("".concat(difference, "%"));
        var previousBundleSizeText = cyan("".concat(Math.round(previousBundleSize), " kB"));
